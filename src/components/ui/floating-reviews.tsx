...
const FloatingReviews: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Only proceed if component is mounted and window is available
  ...
  // Don't render until component is mounted and window is available
  if (!isMounted || typeof window === 'undefined' || visibleReviews.length === 0) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 pointer-events-none z-40 overflow-hidden">
      {visibleReviews.map((review) => {
        ...
        if (!review.position || typeof review.position.x === 'undefined' || typeof review.position.y === 'undefined') {
          return null;
        }
